	...
	DEPTHS:
		for depth := 0; depth < maxdepth; depth++ {
			if len(det.History)-1 < offset+depth {
				logger.Infof("scan %s has only %d histories (%d doesn't exist)", s.ScanID, len(det.History), offset+depth+1)
				break DEPTHS
			}
		...
		STATUSCHECK:
			...
			err = vm.ProcessCSVRow(filename, tgtFilename, vm.FilterKeepAll)
			if err != nil {
...
func (vm *VM) ScansDetail(cmd *cobra.Command, args []string) {
	...
	logger.Infof(("Getting scans from Tenable.io"))
	scans, err := a.Scans(true, true)
	if err != nil {
	...
	filename := fmt.Sprintf("scandetails.%s.csv", dts)

	csv := fmt.Sprintf("%s\n", cli.Render("ScansDetailHeaderCSV", nil))
	for i := range scans {
		details, err := a.ScanDetails(&scans[i], true, true)
		logger.Infof("Getting scan details for '%s' (%s) from Tenable.io", (&scans[i]).Name, (&scans[i]).ScanID)
		if err != nil {
			logger.Fatalf("error: couldn't retrieve details: %s: %+v", scans[i], err)
		}
		row := fmt.Sprintf("%s\n", cli.Render("ScansDetailCSV", map[string]interface{}{"d": details}))
		csv += row
	}

	logger.Infof("Writing all results to file at once.")
	err = ioutil.WriteFile(filename, []byte(csv), 0644)
	if err != nil {
		logger.Fatalf("can't write to file '%s': %+v", filename, err)
	}

	cleanTemplate := `scandetails\.\d+T\d+.csv`
	logger.Infof("keeping a maximum '%d' for template '%s'", maxkeep, cleanTemplate)
	vm.CleanupFiles(`.`, cleanTemplate, maxkeep)
